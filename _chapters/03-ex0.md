...
If you're using a *NIX distribution, you might be able to use your distro's package manager to get Julia. These are a better option for folks who have hardware or dependency issues while compiling Julia from source. Do remember that if you have installed and update your local copy of Julia via the package manager's nightly builds, you need to be aware that they run from a different directory than when you compile from source. 


1. __UBUNTU__: For Ubuntu users, there is a PPA provided for Julia, so all you need to do is get a shell and run the following commands:
...
A PPA (Personal Package Archive) is a personal collection of the latest version of the software not included in Ubuntu by default, usually provided by community maintainers and/or upstream developers maintaining the software. 
