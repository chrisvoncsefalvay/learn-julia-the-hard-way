...
Types may be _abstract_ or _concrete_. You can think of an abstract type as a type that is intended solely to act as supertypes of other types, rather than types of particular objects. Thus, no object that is not a type can have an abstract type as its type. Concrete types are intended to be the types of actual objects. They are usually subtypes of abstract types.
...
First of all, we need a type _definition_. We also know that this would be a _composite_ type, since we want it to hold three values - one for each of pounds, shillings and pence. We also know that these would have to be integers.
