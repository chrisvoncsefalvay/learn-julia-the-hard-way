...
You mght be familiar by now with string and character literals from the introductory chapter, which introduced some literals, or from other programming languages. A string literal is surrounded by _double quotes_: `" string "`. Within the string, you can escape a double-quote using a backslash:
...
This is a regex literal that matches (English) wovels. Julia recognises regex literals as the type `regex`:
